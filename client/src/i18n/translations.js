  ...
  en: {
    ...
    settings: {
      ...
      apiKey: {
        ...
        description: 'You have two options: use a shared API key for quick start (may be slower), or use your own API key for optimal performance.',
        sharedKeyActive: 'is currently active (hidden for security)',
        sharedKeyNote: 'You are currently using a shared key. For better performance, consider using your own key.',
        enterCustomKey: 'Enter your own API key below',
        serverKeyActive: 'Using Server-Managed API Key',
        serverKeyNote: 'API key is securely managed on the server. No key is stored in your browser.',
        useServerKey: 'Use Server Key',
        serverKeyInfo: '⚡ Instant access • 🔒 Secure • May have rate limits during high traffic',
        status: {
          valid: '✅ API Key is valid and working!',
          invalid: '❌ Invalid API Key',
          invalidFormat: 'Key format looks invalid',
          formatOk: 'Key format looks OK',
          saved: '✅ API Key saved and synced to extension!',
          savedNoExt: '✅ API Key saved (extension not available)',
          cleared: '🗑️ API Key cleared and synced to extension',
          clearedNoExt: '🗑️ API Key cleared (extension not available)',
          testing: 'Testing API key...',
          failed: 'Key failed validation',
          syncedFrom: '🔄 API Key synced from extension',
          updatedFrom: '🔄 API Key updated from extension',
          sharedActivated: 'activated and saved! Note: Shared keys may have slower performance.',
          sharedActivatedNoExt: 'activated and saved! (extension not available) Note: Shared keys may have slower performance.',
          serverKeyActivated: '✅ Server key activated! Synced with extension.',
          serverKeyActivatedNoExt: '✅ Server key activated! (Extension not detected)',
          accessDenied: '❌ API Key is invalid or access denied',
          unexpectedResponse: '⚠️ Unexpected response:',
          networkError: '❌ Network error while testing key',
        },
        quickStart: {
          title: 'Quick Start - Server Key',
          description: 'Click the button below to use our server-managed API key. No setup required! The key is securely stored on our server and never exposed to your browser.',
        },
        ownKey: {
          title: 'Optimal Performance - Your Own API Key',
          description: 'For faster and more reliable performance, use your own Gemini API key. Your key is stored locally and never uploaded to our servers.',
          instructionsTitle: '📝 How to Get Your Free API Key',
          stepByStep: 'Step-by-Step Guide:',
          step1Title: 'Visit Google AI Studio',
          step2Title: 'Sign in with your Google Account',
          step2Desc: 'Use any Google account (Gmail, Workspace, etc.)',
          step3Title: 'Click "Create API Key" button',
          step3Desc: "You'll see it in the main page after signing in",
          step4Title: 'Select or Create a Google Cloud Project',
          step4Desc: 'You can create a new project or use an existing one',
          step5Title: 'Copy your API Key',
          step5Desc: 'It starts with "AIza..." - Click the copy button',
          step6Title: 'Paste the key in the field below',
          step6Desc: 'Then click "Save API Key" to start using LogiCheck!',
          freeTier: 'Free Tier: Google provides generous free quotas for Gemini API. No credit card required to start!',
        },
        security: {
          title: '🔐 Security Note',
          point1: "Your API key is stored only in your browser's localStorage",
          point2: "The key is sent directly to Google's Gemini API from your browser",
          point3: 'LogiCheck servers never see or store your API key',
          point4: 'You can clear your key at any time',
        },
      },
      theme: {
        title: 'Theme',
  ...
  id: {
    ...
    settings: {
      ...
      apiKey: {
        ...
        testing: 'Menguji...',
        clear: 'Hapus',
        useCustomKey: 'Gunakan Key Sendiri',
        yourApiKey: 'API Key Anda',
        description: 'Anda memiliki dua opsi: gunakan API key bersama untuk memulai dengan cepat (mungkin lebih lambat), atau gunakan API key Anda sendiri untuk performa optimal.',
        sharedKeyActive: 'sedang aktif (tersembunyi untuk keamanan)',
        sharedKeyNote: 'Anda saat ini menggunakan key bersama. Untuk performa yang lebih baik, pertimbangkan untuk menggunakan key Anda sendiri.',
        enterCustomKey: 'Masukkan API key Anda sendiri di bawah',
        serverKeyActive: 'Menggunakan API Key Server',
        serverKeyNote: 'API key dikelola dengan aman di server. Tidak ada key yang disimpan di browser Anda.',
        useServerKey: 'Gunakan Key Server',
        serverKeyInfo: '⚡ Akses instan • 🔒 Aman • Mungkin ada batasan saat traffic tinggi',
        status: {
          valid: '✅ API Key valid dan berfungsi!',
          invalid: '❌ API Key tidak valid',
          invalidFormat: 'Format key tampaknya tidak valid',
          formatOk: 'Format key terlihat OK',
          saved: '✅ API Key tersimpan dan disinkronkan ke ekstensi!',
          savedNoExt: '✅ API Key tersimpan (ekstensi tidak tersedia)',
          cleared: '🗑️ API Key dihapus dan disinkronkan ke ekstensi',
          clearedNoExt: '🗑️ API Key dihapus (ekstensi tidak tersedia)',
          testing: 'Menguji API key...',
          failed: 'Key gagal validasi',
          syncedFrom: '🔄 API Key disinkronkan dari ekstensi',
          updatedFrom: '🔄 API Key diperbarui dari ekstensi',
          sharedActivated: 'diaktifkan dan disimpan! Catatan: Key bersama mungkin memiliki kinerja yang lebih lambat.',
          sharedActivatedNoExt: 'diaktifkan dan disimpan! (ekstensi tidak tersedia) Catatan: Key bersama mungkin memiliki kinerja yang lebih lambat.',
          serverKeyActivated: '✅ Key server diaktifkan! Disinkronkan dengan ekstensi.',
          serverKeyActivatedNoExt: '✅ Key server diaktifkan! (Ekstensi tidak terdeteksi)',
          accessDenied: '❌ API Key tidak valid atau akses ditolak',
          unexpectedResponse: '⚠️ Respons tidak terduga:',
          networkError: '❌ Kesalahan jaringan saat menguji key',
        },
        quickStart: {
          title: 'Mulai Cepat - Key Server',
          description: 'Klik tombol di bawah untuk menggunakan API key yang dikelola server kami. Tidak perlu setup! Key disimpan dengan aman di server kami dan tidak pernah terekspos ke browser Anda.',
        },
        ownKey: {
          title: 'Performa Optimal - API Key Anda Sendiri',
          description: 'Untuk performa yang lebih cepat dan lebih andal, gunakan API key Gemini Anda sendiri. Key Anda disimpan secara lokal dan tidak pernah diunggah ke server kami.',
          instructionsTitle: '📝 Cara Mendapatkan API Key Gratis Anda',
          stepByStep: 'Panduan Langkah demi Langkah:',
          step1Title: 'Kunjungi Google AI Studio',
          step2Title: 'Masuk dengan Akun Google Anda',
          step2Desc: 'Gunakan akun Google apa pun (Gmail, Workspace, dll.)',
          step3Title: 'Klik tombol "Create API Key"',
          step3Desc: 'Anda akan melihatnya di halaman utama setelah masuk',
          step4Title: 'Pilih atau Buat Proyek Google Cloud',
          step4Desc: 'Anda dapat membuat proyek baru atau menggunakan yang sudah ada',
          step5Title: 'Salin API Key Anda',
          step5Desc: 'Dimulai dengan "AIza..." - Klik tombol salin',
          step6Title: 'Tempel key di bidang di bawah',
          step6Desc: 'Kemudian klik "Simpan API Key" untuk mulai menggunakan LogiCheck!',
          freeTier: 'Gratis: Google menyediakan kuota gratis yang murah hati untuk Gemini API. Tidak perlu kartu kredit untuk memulai!',
        },
        security: {
          title: '🔐 Catatan Keamanan',
          point1: 'API key Anda hanya disimpan di localStorage browser Anda',
          point2: 'Key dikirim langsung ke Gemini API Google dari browser Anda',
          point3: 'Server LogiCheck tidak pernah melihat atau menyimpan API key Anda',
          point4: 'Anda dapat menghapus key Anda kapan saja',
        },
      },
      theme: {
        title: 'Tema',
